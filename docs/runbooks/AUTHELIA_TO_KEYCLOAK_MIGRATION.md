...
- The `keycloak-standard-client-scopes` Job backfills the standard Keycloak OIDC scopes for existing realms so the Account Console and Admin Console receive the expected `sub`, role, origin, and ACR token claims.
