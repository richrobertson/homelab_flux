...
- The `keycloak-standard-client-scopes-v3` Job backfills the standard Keycloak OIDC scopes and the `account-console` to `account/manage-account` client scope mapping for existing realms so the Account Console and Admin Console receive the expected `sub`, role, origin, ACR, and account-management token claims.
